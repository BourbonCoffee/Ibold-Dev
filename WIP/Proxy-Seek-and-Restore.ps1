...
Param(
	...
	[ValidateNotNullOrEmpty()]
	[String]$OrgMgmtAdmin = ($env:userprofile + "\Documents\WindowsPowershell\pswd\pswd.xml"),
...
ForEach ($user in $users){
    ...
    If ($users.count -gt 1){
		$p++
		Write-Progress -Id 1 -Activity "Processing $p of $($users.count) total addresses" -Status ("{0:P2}" -f ($p / $($users).Count)) -CurrentOperation $user.Alias
	...
	#If ($user.targetSamAccountName){
		$targetObject = $null
		Out-CMTraceLog -LoggingPreference $htLoggingPreference -Logfile $logFileName -Message "Using alias of $($user.alias) to find a match in the target" -Type Information -WriteBackToHost
		#$targetObject = Get-Recipient -Identity $user.targetSamAccountName -ErrorAction Stop
		$targetObject = Get-Recipient -Identity $user.alias -Verbose:$false #-ErrorAction Stop
	...
	# 	$targetObject = Get-Recipient -Identity $user.alias -ErrorAction Stop
	# }
		...
		If ($ObjectType -eq "All" -or $ObjectType -eq "Mailboxes"){
			If ($addressType){
				...
				If ($addressType[0] -clike "SIP"){
					...
					If (!$IgnoreSIP){ # Checking if we are re-applying SIP addresses
