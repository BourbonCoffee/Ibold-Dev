...
if ($ConnectionInformation.State -eq $null -or $ConnectionInformation.State -eq "Broken") {
...
foreach ($mailbox in $mailboxes) {
    $size = Get-MailboxStatistics $mailbox.PrimarySmtpAddress
    $myObject = [PSCustomObject]@{
        ...
        TotalSizeGB            = ConvertTo-Gb -size $size.TotalItemSize.ToString().Split("(")[0]
        LastUserActionTime     = $size.LastUserActionTime
    ...
    $mailboxStats += $myObject
}


$desktop = Get-KnownFolderPath "Desktop"

if (-not (Test-Path "$home\Desktop\Office365Inventory")) {
    New-Item -Path "$home\Desktop" -Name "Office365Inventory" -ItemType "directory" | Out-Null
}

$mailboxStats | Export-Csv $desktop\Office365Inventory\Mailboxes.csv -NoTypeInformation

$publicFolders = Get-PublicFolderStatistics -ResultSize Unlimited -ErrorAction SilentlyContinue | Select-Object Name, DatabaseName, @{Name = 'Size'; Expr = { [int64]($_.TotalItemSize -split '[\( ]')[3] } }
$publicFolders | Export-Csv $desktop\Office365Inventory\PublicFolders.csv -NoTypeInformation

Get-AcceptedDomain | Export-Csv $desktop\Office365Inventory\Domains.csv -NoTypeInformation
