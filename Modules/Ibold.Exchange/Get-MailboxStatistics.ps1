...
.EXAMPLE
  Get-MailboxStatistics.ps1 -sharedMailboxes only
  Get only the shared mailboxes
.EXAMPLE
  Get-MailboxStatistics.ps1 -sharedMailboxes no
  Get only the user mailboxes
.EXAMPLE
  Get-MailboxStatistics.ps1 -archive:$false
  Get the mailbox size without the archive mailboxes
.EXAMPLE
  Get-MailboxStatistics.ps1 -CSVpath c:\temp\report.csv
  Store CSV report in c:\temp\report.csv
.EXAMPLE
  Get-MailboxStatistics.ps1 | Format-Table
...
param(
  ...
  [string]$CSVpath = "$([Environment]::GetFolderPath('Desktop'))\Mailbox-Statistics.csv"
)
