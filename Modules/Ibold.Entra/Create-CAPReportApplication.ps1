...
if ( $UseClientSecret) {
    ...
    Write-Host "The following command can be used to run the assessment with a Client Secret:`n.\Get-CAPDetailedReport.ps1 -TenantID $tenantID -ClientID $($appReg.appID) -Secret $($appSecret.value)"
} elseif ($UseCertificate) {
    ...
    Write-Host "The following command can be used to run the assessment with a Certificate:`n.\Get-CAPDetailedReport.ps1 -TenantID $tenantID -ClientID $($appReg.appID) -CertificateThumbprint $thumbprint"
} else {
    ...
    Write-Host "The following command can be used to run the assessment with delegated permissions:`n.\Get-CAPDetailedReport.ps1 -TenantID $tenantID -ClientID $($appReg.appID)"
}
Pause
Clear-Host